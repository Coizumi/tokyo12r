...
予想印、指数、距離補正、クラス順位ボーナス、買い目生成、的中判定などの予想ロジックは `forcast_for_jra.md` を唯一の参照先とし、この文書にはロジック詳細を記載しない。
...
予想・結果・全頭指数ページの各レース見出しは、開催場名とレース番号を連結した `中山1R`、`阪神12R` の形式で統一する。番号ラベルは改行せず、背景の左右内側余白を各 `10px` として視認性を確保する。隣のレース名・条件は幅に合わせて折り返す。リンク先のアンカーIDとデータ内のレース番号は変更しない。
...
- レースカード: レース番号、レース名、条件、発走時刻、予想印、結果ページへのリンク、全頭指数ページへのリンク
- フッター: 年齢注意などの補足

予想印は印、馬番、馬名、総合力指数、人気を同じ行に表示する。総合力指数は小数1桁の `00.0` 形式で表示する。馬名は省略表示にせず、カード幅に応じて折り返して全文を表示する。
...
結果ページリンクの右に、全頭指数ページの該当レースへ移動するリンクを表示する。

```text
/resultYYYYMMDD.html#race-<venue-key>-<race-no>
/scoresYYYYMMDD.html#race-<venue-key>-<race-no>
```

結果が未確定でもリンク先のアンカーは結果ページ側と全頭指数ページ側に存在させる。これにより、結果確定前後でリンクの有無やジャンプ先が変わらないようにする。

### 全頭指数ページ

全頭指数ページは、対象開催日の全出走馬の総合力指数を開催場ごと、レースごとに表示する。
URLは `scoresYYYYMMDD.html` とする。
...
- サマリー: 対象日、更新時刻
- 本文: 開催場ごとの1本の縦方向の柱
- レース単位: レース番号、レース名、条件、発走時刻、全頭指数表

全頭指数表は、総合力指数の高い順に以下を表示する。
...
- レース番号: 開催場名付きの `中山1R` から `中山12R` など
- レース名
- 条件
- 発走時刻
- 1着、2着、3着の馬番と馬名
...
- 金曜 22:10 JST
- 土曜 08:45, 09:15, 09:45 ... 20:15, 20:45, 22:10 JST
- 日曜 08:45, 09:15, 09:45 ... 20:15, 20:45 JST
- 月曜 08:45, 09:15, 09:45 ... 20:15, 20:45 JST
- 火曜 08:45, 09:15, 09:45 ... 20:15, 20:45 JST

金曜22:10と土曜22:10は翌日分の予想を生成する。翌営業開催を拾うため、対象日から最大4日先まで公式出馬表を確認する。
土曜の日中、日曜、月曜、火曜の更新では対象日を確認する。

月曜・火曜は開催がある場合のみ実処理する。08:45 JSTの開催チェックで開催がないと判定した場合は、その日の `no-race` マーカーを作成し、以後の同日スケジュールは正常終了する。

更新のたびに全開催場の人気順を取得し、未発走レースの公開データを更新する。各レースは発走時刻前の最後の更新で公開した予想印と、その印から生成した買い目を確定し、発走時刻以降の更新では既存の `public-dataYYYYMMDD.json` から復元して変更しない。発走後も出走表と結果は再取得し、結果・払戻・的中判定を更新する。

公式サイトへの連続アクセスを抑えるため、JRA公式取得時の既定待機時間は `1.2` 秒とする。既存の `public-dataYYYYMMDD.json` で全レースの結果が確定済み、かつ公開生成物より新しい生成入力ファイルがない場合、その日の後続更新は公式取得、SQLite更新、R2保存、Cloudflare Pagesデプロイを行わず正常終了する。
...
この文書では、予想ロジックの式、係数、印の選定手順、買い目の生成ルール、的中判定条件を定義しない。グレード競走を含むレース条件別の指数適用範囲も `forcast_for_jra.md` を参照し、実装変更時は同文書を更新する。
...
4. `forcast_for_jra.md` に従って公開用予想、買い目、結果判定データを生成し、発走済みレースの予想印と買い目は直前の公開JSONから復元して固定
5. SQLiteへupsert
6. 公開用特徴量ファイルを出力
