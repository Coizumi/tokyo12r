...
TOKYO12Rは、公開配信をCloudflare Pagesへ寄せ、データ収集と指数計算をWebARENA Indigo VPSに集約する。
...
flowchart LR
  ...
  batch --> dispatch["GitHub Actions dispatch<br/>必要時のみ"]
...
- WebARENA Indigo VPS: JRAデータ収集、SQLite蓄積、指数計算、公開用特徴量生成を担当する
- GitHub Actions: 静的HTML生成とCloudflare Pagesデプロイを担当する
...
- Cloudflare Cronの曜日指定は、既存運用と同じ数値形式で管理する。金曜は `5`、火曜は `2` とする。
