...
現時点では `/etc/tokyo12r-feature-pipeline.env` は未設定とし、VPSからのGitHub Actions dispatchは無効のままにする。
そのため、VPSバッチはSQLite/JSON生成までを担当し、Cloudflare Pagesへの反映は手動または既存GitHub Actions運用で実施する。
...
- [ ] GitHub Actions dispatch用tokenを `/etc/tokyo12r-feature-pipeline.env` に設定する
